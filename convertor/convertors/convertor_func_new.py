...
"""
    ...
    def write_rule(self, name, commands, depfile, deps, other_attr):
        ...
        self.write_line(f"void {name}_rule() {{")
        self.incr_indent()
    ...
    def write_build(self, _in, order_in, implicit_in, _out, implicit_out, rule, vars: dict):
        ...
        # if rule == "rule::phony":
        self.write_line(f"void build{self.build_count}_create() {{")
        # else:
        #     self.write_line(f"void build{self.build_count}_create(rule {rule}) {{")
    ...
    def __write_body(self):
        ...
        for item in self.declare_mapping:
            name = item["name"]
            if item["type"] == "rule":
                self.write_line(f"{name}_rule();")
            elif item["type"] == "build":
                # rule = item["rule"]
                self.write_line(f"{name}_create();")
            self.newline()
