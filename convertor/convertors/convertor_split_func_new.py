...
"""
        self.begin_main = f"""void manifest{index}() {{
"""
    ...
    def write_build(self, _in, order_in, implicit_in, _out, implicit_out, rule, vars: dict):
        ...
        self.write_line(f"static void build{self.build_count}_create() {{")
        self.incr_indent()
    ...
    def write_body(self):
        for index, rule in enumerate(self.build_rule_map):
            build_num = index + 1
            self.write_line(f"build{build_num}_create();")
            self.newline()
...
"""
        self.begin_main = f"""void manifest() {{
"""
    ...
    def write_sub_manifest(self):
        self.write_line(f"void build_rule();")
        for i in range(self.max_index):
            self.write_line(f"void manifest{to_hex(i)}();")
        self.newline()
    ...
    def write_manifest(self):
        ...
        for i in range(self.max_index):
            self.write_line(f"manifest{to_hex(i)}();")
...
class CompileScriptWriter(FileWriter):
    ...
    def write_script(self):
        self.write_line(f"clang++ {optimizer} -fPIC -c manifest_rule.cc -o rule.o && echo \"rule compiled.\"")

        commands = FileWriter("output/commands.txt")
        commands.open()
        for i in range(self.max_index):
            commands.write_line(f"clang++ {optimizer} -fPIC -c manifest{to_hex(i)}.cc -o manifest{to_hex(i)}.o && echo \"{to_hex(i)} compiled.\"")
        commands.close()
